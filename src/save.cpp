...
void Save::on_saveImagesButton_clicked() {
    QString filename;
    filename = QFileDialog::getSaveFileName(this, tr("Select image file to save to"), ui->imagesFileLine->text(), "*.png");
    if (0 == filename.length()) return;
    close();
    ui->imagesFileLine->setText(filename);
    saveFilenames();
    QPixmap pm = QPixmap::grabWidget(MainWindow::instance->getUI()->tracks);
    pm.save(filename);
}
