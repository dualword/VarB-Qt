...
void TrackGraph::paintLine(QPainter &painter, int last_x, int h) {
    painter.drawLine(label_offset + last_x, top + height - h, label_offset + last_x, top + height);
}
/*
void TrackGraphFST::paintRect(QPainter &painter, int x1, int x2, int h) {
    qDebug() << "paintRect" << x1 << h;
    painter.drawRect(label_offset + x1, top + height - h, x2-x1, h - 1);
}

void TrackGraphFST::paintLine(QPainter &painter, int last_x, int h) {
    qDebug() << "paintLine" << last_x << h;
    painter.drawLine(label_offset + last_x, top + height - h, label_offset + last_x, top + height);
}
*/
...
void TrackGraphTD::paintLine(QPainter &painter, int last_x, int h) {
...
//    qDebug() << "TrackGraphFST::calculateData start" << chr;
    ...
    for (int i = 0; i < len; i++) {
        ...
        foreach (QString gp, var->groups.keys()) {
            ...
            int alleles = 0, samples = 0, hetero = 0;
...
//        qDebug() << "Final: " << i << d[i].data << groups << fst << nbar;
    }
    size[chr] = len;
    data[chr] = d;
//    qDebug() << "TrackGraphFST::calculateData end";
